use crate::pb::mydata::v1::{Burn, InitializedAccount, Mint, Output, Transfer};
...
pub fn process_compiled_instruction(
    ...
    process_inner_instructions(
        output, inst_index, meta, accounts, trx_hash, timestamp, parameters,
    );
}
            ...
            inst.instructions
                ...
                .for_each(|inner_instruction| {
                    match process_token_instruction(
                        ...
                        Err(err) => {
                            panic!(
                                "trx_hash {} filtering inner instructions: {}",
                                trx_hash, err
                            )
                        }
                        Ok(()) => {}
...
fn process_token_instruction(
    ...
    match TokenInstruction::unpack(&data) {
        ...
        Ok(instruction) => match instruction {
            ...
            | TokenInstruction::MintToChecked { amount: amt, .. } => {
            ...
            | TokenInstruction::BurnChecked { amount: amt, .. } => {
